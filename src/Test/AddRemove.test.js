...
describe('Adding items', () => {
  ...
  test('test if newTodolist is adding', () => {
    expect(newTodoList()).toEqual([
      { completedTask: false, description: 'completed tasks', id: 1 },
      { completedTask: false, description: 'send tasks', id: 2 },
      { completedTask: false, description: 'completed', id: 3 },
      { completedTask: false, description: 'start next task', id: 4 },
      { completedTask: false, description: undefined, id: 5 },
    ]);
  });
});
...
describe('Editing items in todos', () => {
  ...
  test('Testing if todos are edited in array', () => {
    expect(edit(3, 'Cristina')).toEqual([
      { completedTask: false, description: 'completed tasks', id: 1 },
      { completedTask: false, description: 'send tasks', id: 2 },
      { completedTask: false, description: 'Cristina', id: 3 },
      { completedTask: false, description: 'start next task', id: 4 },
      { completedTask: false, id: 5 },
    ]);
  });
...
describe('check if to-do list is complete', () => {
  ...
  test('check is complete', () => {
    expect(checkComplete(3)).toEqual([
      { completedTask: false, description: 'completed tasks', id: 1 },
      { completedTask: false, description: 'send tasks', id: 2 },
      { completedTask: true, description: 'completed', id: 3 },
      { completedTask: false, description: 'start next task', id: 4 },
      { completedTask: false, id: 5 },
    ]);
  });
...
describe('Clearing all completed items', () => {
  test('check if clearAll is a function', () => {
    expect(typeof clearAll).toBe('function');
  });
  test('Testing if Completed To-Dos are removed', () => {
